...
const ProfilePage = () => {
  ...
  const handlePasswordSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      await authApi.changePassword(passwordForm.currentPassword, passwordForm.newPassword, passwordForm.confirmPassword);
  ...
  return (
    <div className="space-y-6">
      <PageHeader title="Profile & Settings" subtitle="Review your account details, update security, and manage workspace preferences." />
      ...
      <div className="rounded-[28px] border border-slate-200 bg-white p-6 shadow-sm">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          <div>
            <p className="text-sm font-semibold uppercase tracking-[0.24em] text-slate-500">Account overview</p>
            <h3 className="mt-2 text-xl font-semibold text-slate-900">{user?.fullName || 'Your profile'}</h3>
            <p className="mt-2 text-sm text-slate-600">{user?.email || 'Manage your own profile details and access level.'}</p>
          </div>
          <div className="flex flex-wrap gap-2">
            <span className="rounded-full bg-slate-100 px-3 py-1 text-sm font-semibold text-slate-700">Role: {roleLabel}</span>
            <span className={`rounded-full px-3 py-1 text-sm font-semibold ${user?.isActive ? 'bg-emerald-100 text-emerald-700' : 'bg-amber-100 text-amber-700'}`}>
              {user?.isActive ? 'Active account' : 'Inactive account'}
            </span>
          </div>
        </div>
        <p className="mt-4 text-sm text-slate-500">Only your own profile can be edited from this workspace. Changes are validated on both the client and server before they are saved.</p>
      </div>
